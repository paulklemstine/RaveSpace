...
import fragmentShader from "../shaders/acidwarp.frag";

const METADATA = SCENE_REGISTRY.find((s) => s.id === "acidwarp")!;

export class AcidWarp implements ParameterizedScene {
  ...
  setParams(values: ParamValues): void {
    ...
    if (values.colorDensity !== undefined) this.uniforms.uColorDensity.value = values.colorDensity as number;
    ...
    if (values.flowSpeed !== undefined) this.uniforms.uFlowSpeed.value = values.flowSpeed as number;
  }

  getParams(): ParamValues {
    return {
      speed: this.uniforms.uSpeed.value as number,
      warpIntensity: this.uniforms.uWarpIntensity.value as number,
      colorDensity: this.uniforms.uColorDensity.value as number,
      audioReactivity: this.uniforms.uAudioReactivity.value as number,
      colorShift: this.uniforms.uColorShift.value as number,
      flowSpeed: this.uniforms.uFlowSpeed.value as number,
    };
  }
